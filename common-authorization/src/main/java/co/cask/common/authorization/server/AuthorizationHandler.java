 ...
 */
@Path("/v1")
public class AuthorizationHandler extends AbstractHttpHandler {
  ...
  @Inject
  public AuthorizationHandler(ACLStore aclStore) {
  ...
  @POST
  @Path("/acls/{object-type}/{object-id}/{subject-type}/{subject-id}/{permission}")
  public void setACL(HttpRequest request, HttpResponder responder,
                     @PathParam("object-type") String objectType, @PathParam("object-id") String objectId,
                     @PathParam("subject-type") String subjectType, @PathParam("subject-id") String subjectId,
                     @PathParam("permission") String permission) {
    ACLEntry aclEntry = new ACLEntry(new ObjectId(objectType, objectId),
                                     new SubjectId(SubjectType.valueOf(subjectType), subjectId),
                                     permission);
