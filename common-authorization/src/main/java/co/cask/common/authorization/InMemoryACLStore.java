...
public class InMemoryACLStore implements ACLStore {
  ...
  public boolean write(ACLEntry entry) {
    ...
    Set<String> value = getPermissions(key);
    if (value == null) {
  ...
  public List<ACLEntry> read(ObjectId objectId, SubjectId subjectId) {
    ...
    Set<String> permissions = getPermissions(key);
  ...
  private Set<String> getPermissions(Key key) {
    if (!store.containsKey(key)) {
      store.put(key, Sets.<String>newHashSet());
    }
